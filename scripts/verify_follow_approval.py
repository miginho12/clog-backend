...
async def run(session: AsyncSession) -> None:
    ...
    pubA = make_user("pubA", is_public=True)
    pubB = make_user("pubB", is_public=True)
    priv = make_user("priv", is_public=False)
    req = make_user("requester", is_public=True)
    session.add_all([pubA, pubB, priv, req])
    await session.flush()

    print("\n[공개 계정 팔로우 → 즉시 accepted]")
    st = await s.follow(follower_id=pubA.id, following_id=pubB.id)
    check("반환 상태 accepted", st == "accepted")
    check("pubB 팔로워 수 1", await repo.count_followers(user_id=pubB.id) == 1)

    print("\n[비공개 계정 팔로우 → pending]")
    st = await s.follow(follower_id=pubA.id, following_id=priv.id)
    check("반환 상태 pending", st == "pending")
    check("priv 팔로워 수 0 (아직 수락 전)", await repo.count_followers(user_id=priv.id) == 0)
    check("pubA 팔로잉에 priv 미포함(accepted만)",
          priv.id not in await repo.following_ids(follower_id=pubA.id, user_ids=[priv.id]))
    check("priv 요청함에 pubA 있음", await repo.count_pending_requests(user_id=priv.id) == 1)
    ...
    print("\n[요청 수락]")
    await s.accept_request(owner_id=priv.id, requester_id=pubA.id)
    ...
    check("get_follow_status=accepted",
          await s.get_follow_status(follower_id=pubA.id, following_id=priv.id) == "accepted")
    ...
    await expect(
        s.accept_request(owner_id=priv.id, requester_id=pubB.id),
        FollowRequestNotFound,
